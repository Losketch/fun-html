...
function playSound(soundSequence, onComplete=null) {  
  ...
  function _play(soundSequence) {
    ...
    if (interval == 0) {start()}
    else {playerTimer = setTimeout(start, interval)}
  }
...
function encodeMorse(text, wpm=20, wordInterval=3, dashDotLenRatio=3) {
  ...
  for (const i of text.toArray()) {
    for (let j = Math.floor(logBase(i, 2)); j >= 0; j--) {
      const interval = unitTime * (isWordStart ? 3 : 1) * (isFirstLetter ? 0 : 1) * 1000
      const duration = ((i >> j & 1) * (dashDotLenRatio - 1) + 1) * unitTime * 1000;
...
encodeButton.addEventListener('click', () => {
  ...
  const soundSequence = encodeMorse(input.value, +wpmSlider.value, +wordIntervalSlider.value, +dashDotLenRatioSlider.value)
...
stopButton.addEventListener('click', () => {if (stopper) stopper()});
